...
// ---------------- end exif data --------------------------------------
  ?> 
 <style type="text/css">
 <?php if(!isset($style_full)){
                        include('../'.$ki_template.'/stylesheet/koschtit_gallery_fullimg.css');
                        $style_full = 1;
 }
 ?>
</style>
...
$commquery = $ki_db->query("SELECT * FROM ".$db_prefix ."koschtit_gallery_admin_comment WHERE folder_name='".$gallery."' AND image_name='".$file."' AND language_id='".$ki_session_language."'");
$result = $commquery->row;
...
<img id='thepicture' style='margin-left:-<?php echo floor($x*0.5) ?>px; margin-top:-<?php echo floor($y*0.5) ?>px; width:<?php echo $x ?>px; height:<?php echo $y ?>px;' alt='<?php echo $srcfile ?>' onclick='kiv.goon(1)' class='ki_fullimg'/>
...
if($ki_comments == 1){
	if($pwok == 0){
		if($comm != ""){
			$comm = nl2br($comm);
                                                                     echo "\r\n";
			echo '<div id="thecomment" style="display:none;" class="ki_thecomment">'.stripslashes($comm).'</div>';
...
$vcommquery = $ki_db->query("SELECT * FROM ".$db_prefix ."koschtit_gallery_viewercomment WHERE folder_name='".$gallery."' AND image_name='".$file."'");
$vcomm_dbase = $vcommquery->row;

	$value = 0;
	if(isset($vcomm_dbase['comment'])){
		$value = 1;
	}
...
if($pwok == 1 && $ki_th_to_square == 1 && $ki_thumbs_to_disk == 1){
	...
	echo "\r\n<div id='square_selector' style='position:absolute; left:".$ki_bord_size."px; top:".$ki_bord_size."px; width:".$x."px; height:".$y."px; z-index:1001; padding:2px; background:#000000; overflow:hidden; cursor:pointer; display:none;'>\r\n<img src='".$srcfile."' style='visibility:hidden;' onload=\"this.style.visibility='visible';\" />\r\n<div id='square_1' style='background:#000000; position:absolute; left:2px; top:2px; overflow:hidden;".$stylestring1."'></div><div id='square_2' style='background:#000000; position:absolute; right:2px; bottom:2px; overflow:hidden;".$stylestring2."'></div></div>";
}
?>
